...
> **Synced copy.** Authoritative version lives in the app repo at
> `Erpnext-Sbca/docs/PHAROH_API_CONTRACT.md` — it travels with the consumer
> code. This copy is here so the knowledge base is self-contained; if the two
> drift, the repo copy wins.
...
This document replaces three implementation briefs that have served their
purpose now the endpoints are built and deployed:

- `Pharoh_GetStockLevels_Endpoint_Prompt.txt`
- `Pharoh_DisableQtyTracking_Endpoint_Prompt.txt`
- `Pharoh_StockAdjustment_Endpoint_Prompt.txt`

They can be deleted. The other `Pharoh_*_Endpoint_Prompt.txt` files
(JournalEntry, Cancellation, Reconciliation) are unrelated — different
controllers — and are left as-is.
