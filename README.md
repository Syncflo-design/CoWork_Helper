...
| [`erpnext_sbca` (overview)](projects/erpnext_sbca/ASSESSMENT.md) | **CRITICAL — Russell's Sage ↔ ERPNext bridge, written by Doreen (9t9it).** Two-way sync with Sage Business Cloud Accounting: pulls items, prices, stock-on-hand, suppliers, accounts, sales orders, purchase orders FROM Sage; pushes item price changes BACK to Sage. Per-Company credentials in Settings. On nesterp it's installed but not yet switched on (Settings panel still empty as of 2026-05-12). Doc covers what the app does in plain English, the per-Company credential model, current state, the implied Python filesystem layout, and the MCP audit recipe. **Do not stop the jobs / uninstall.** |
