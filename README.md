...
| 2026-05-06 | [`insights-dashboard-client-script-styling.md`](gotchas/2026-05-06-insights-dashboard-client-script-styling.md) | Insights v3 dashboards have no `custom_css` field — style them by injecting a `<style>` tag via a Client Script scoped to `Insights Dashboard v3`, targeting `.vue-grid-item:nth-child(N)`. Needs `!important` (Insights inlines styles), a `setTimeout` wrap (fires before the DOM renders), and a hard refresh; reordering cards breaks the `:nth-child()` numbers |
...
| 2026-05-10 | [`frappe-v16-page-api-drift.md`](gotchas/2026-05-10-frappe-v16-page-api-drift.md) | Custom desk Page JS ported v15→v16 fails with three sequential errors: (a) `wrapper.main` is undefined in modern desk — use `page.body` instead, (b) `page.body` is a jQuery object, not a DOM element — wrap in `$(...)` and use `.append()`, (c) `frappe.client.get_list` throws "Field not permitted in query: <field>" because v16 tightened field-level perms — drop offending fields (e.g. `item_code` on Work Order) or use a backend whitelisted method. **Variant 2 (2026-05-11):** page chrome renders fine but body is empty with NO console error — `$(wrapper).find('.some-class')` against a class nothing creates returns empty jQuery collection, every subsequent `.html()` and `.on()` silently no-ops. Create your container explicitly inside `page.body` |
...
| 2026-05-12 | [`frappe-chatty-cron-on-unconfigured-third-party-app.md`](gotchas/2026-05-12-frappe-chatty-cron-on-unconfigured-third-party-app.md) | Third-party Frappe apps can ship 10+ `frequency: "All"` Scheduled Job Type entries that fire every cron tick even when the app is unconfigured. When taking over an unknown bench, always `frappe_list("Scheduled Job Type", filters=[["method","like","%<app>%"]])` per app and stop or uninstall what you don't need. Apps with tiny DocType surface area but lots of scheduler entries are usually sync integrations — easy to miss in a DocType-only audit |
| 2026-05-13 | [`frappe-whitelisted-action-needs-return-value.md`](gotchas/2026-05-13-frappe-whitelisted-action-needs-return-value.md) | A `@frappe.whitelist()` method that returns `None` (no `return`) sends `{"message": null}` over the wire — the client-side `if (r.message)` branch silently no-ops. Button appears broken, but the DB write actually succeeded. Always return a truthy payload (`return True` minimum, or a dict) from any whitelisted method called by a button + success callback |
| 2026-05-13 | [`pharoh-copilot-prompt-style.md`](gotchas/2026-05-13-pharoh-copilot-prompt-style.md) | GitHub Copilot gives poor results for Pharoh .NET endpoint prompts written casually — it needs the exact structured format (BACKGROUND / WHAT NEEDS TO BE BUILT / FIELD MAPPING table / RESPONSE / ALREADY DONE / numbered YOUR TASK with the Swagger update last). Use real example values in the JSON bodies, always name the reference controller, or have Opus generate the prompt from the JournalEntry example |
| 2026-05-14 | [`cowork-edit-tool-flips-lf-to-crlf.md`](gotchas/2026-05-14-cowork-edit-tool-flips-lf-to-crlf.md) | Cowork's Edit/Write tool can silently rewrite an existing LF repo file as CRLF, so `git diff` shows whole-file churn for a tiny edit. It is inconsistent (some files in the same session stay LF) — check every edited file: out-of-proportion `git diff --stat` churn = EOL flip, confirm with `git diff --ignore-cr-at-eol`, strip the CRs with `sed`. Always `git diff --stat` after an Edit/Write |
| 2026-05-14 | [`custom-app-hook-feedback-loop.md`](gotchas/2026-05-14-custom-app-hook-feedback-loop.md) | A custom app that both hooks a doctype via `doc_events` AND programmatically creates documents of that same doctype trips its own hooks (erpnext_sbca's reconciliation job creates Journal Entries that the push-to-Sage `on_submit` hook would send straight back — feedback loop). Fix: tag internally-created docs with a recognisable marker (a `cheque_no` prefix, not `doc.flags` — flags do not survive `frappe.enqueue`) and bail early in the hook |
| 2026-05-14 | [`erpnext-party-group-must-be-leaf.md`](gotchas/2026-05-14-erpnext-party-group-must-be-leaf.md) | A Customer/Supplier can only be assigned to a LEAF `Customer Group`/`Supplier Group` (`is_group = 0`), never a group node or the tree root — raw assignment throws `Cannot select a Group type` and the record is silently skipped. When syncing external categories, resolve to or create leaf groups (see `ensure_party_group`); same rule for Territory and Item Group |
